...
class SpringBootAdminReactiveApplicationDiffblueTest {
  /**
   * Test {@link
   * SpringBootAdminReactiveApplication#securityWebFilterChainPermitAll(ServerHttpSecurity)}.
   *
   * <ul>
  ...
  void testSecurityWebFilterChainPermitAll_thenReturnMatcherSecurityWebFilterChain()
    ...
    // Arrange
    SpringBootAdminReactiveApplication springBootAdminReactiveApplication =
        new SpringBootAdminReactiveApplication(new AdminServerProperties());

    // Act
    SecurityWebFilterChain actualSecurityWebFilterChainPermitAllResult =
        springBootAdminReactiveApplication.securityWebFilterChainPermitAll(
   ...
   *
   * <p>Method under test: {@link SpringBootAdminReactiveApplication#notifier()}
   */
  @Test
  ...
  void testNotifier() throws AssertionError {
    // Arrange and Act
    Notifier actualNotifierResult =
        new SpringBootAdminReactiveApplication(new AdminServerProperties()).notifier();
    Mono<Void> actualPublisher =
        actualNotifierResult.notify(new InstanceDeregisteredEvent(InstanceId.of("42"), 1L));
   ...
   * <ul>
   *   <li>Given {@link
   *       SpringBootAdminReactiveApplication#SpringBootAdminReactiveApplication(AdminServerProperties)}
   *       with adminServer is {@code null}.
   * </ul>
   *
   * <p>Method under test: {@link SpringBootAdminReactiveApplication#notifier()}
   */
  @Test
  @DisplayName(
      "Test notifier(); given SpringBootAdminReactiveApplication(AdminServerProperties) with adminServer is 'null'")
  @Tag("ContributionFromDiffblue")
  @ManagedByDiffblue
  @MethodsUnderTest({"Notifier SpringBootAdminReactiveApplication.notifier()"})
  void testNotifier_givenSpringBootAdminReactiveApplicationWithAdminServerIsNull()
      throws AssertionError {
    // Arrange and Act
