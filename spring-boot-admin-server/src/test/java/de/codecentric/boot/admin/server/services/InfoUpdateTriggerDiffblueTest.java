...
class InfoUpdateTriggerDiffblueTest {
   ...
   */
  @Test
  @DisplayName("Test handle(Flux); then return Collection")
  @Tag("ContributionFromDiffblue")
  @ManagedByDiffblue
  @MethodsUnderTest({"Publisher InfoUpdateTrigger.handle(Flux)"})
  void testHandle_thenReturnCollection() {
    // Arrange
    Builder builder = mock(Builder.class);
    when(builder.build()).thenReturn(mock(WebClient.class));
    InstanceWebClient instanceWebClient = InstanceWebClient.builder().webClient(builder).build();
    ...
    assertTrue(actualHandleResult instanceof Collection);
    assertTrue(((Collection<Void>) actualHandleResult).isEmpty());
